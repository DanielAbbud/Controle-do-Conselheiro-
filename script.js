...
// =========================================
window.navegar = (aba) => {
    ['config', 'avaliar', 'dashboard', 'admin', 'requisitos'].forEach(id => { const el = document.getElementById('sec-' + id); if (el) el.classList.add('hidden'); });
...
const btnReq = document.getElementById('nav-requisitos'); if (btnReq) btnReq.onclick = () => navegar('requisitos');
...
// =========================================
// REQUISITOS E ACAMPAMENTO (GAMIFICADO)
// =========================================
...
window.carregarCategoriasAdmin = async () => {
    const selectCat = document.getElementById('novo-req-categoria');
    if (!selectCat) return;
    try {
        const snap = await getDocs(query(collection(db, "requisitos_cadastrados")));
        let categorias = new Set();
        snap.forEach(doc => { if (doc.data().categoria) categorias.add(doc.data().categoria); });
        ...
        if (valorAtual && categorias.has(valorAtual)) selectCat.value = valorAtual;
    } catch (e) { console.error("Erro ao carregar categorias", e); }
}

window.adicionarNovaCategoria = async () => {
    const { value: novaCat } = await Swal.fire({
        title: 'Nova Categoria',
        input: 'text',
        inputPlaceholder: 'Ex: MISSIONÁRIOS',
        showCancelButton: true,
        confirmButtonText: 'Adicionar',
        cancelButtonText: 'Cancelar'
    });
    if (novaCat && novaCat.trim() !== "") {
        const nomeCat = novaCat.trim().toUpperCase();
        const selectCat = document.getElementById('novo-req-categoria');
        let opt = document.createElement('option');
        opt.value = nomeCat; opt.text = nomeCat;
        selectCat.add(opt);
        selectCat.value = nomeCat;
    }
}
...
window.carregarRequisitosDisponiveis = async () => {
    const divLista = document.getElementById('lista-requisitos-cards');
    if (!divLista) return;
    ...
    try {
        ...
        document.getElementById('acamp-texto-pontos').innerText = `${pontosConquistados} de ${META_PONTOS} pontos`;
        ...
        if (porcentagemFloat >= 80) {
            ...
            dispararConfetes(); // CHUVA DE CONFETES SE BATER 5 ESTRELAS! 🎉
        }
        else if (porcentagemFloat >= 60) txtEstrelas = "⭐⭐⭐⭐ 4 ESTRELAS";
        else if (porcentagemFloat > 0) txtEstrelas = "⭐⭐⭐ 3 ESTRELAS";
        ...
        snap.forEach(doc => {
            ...
            if (d.categoria !== catAtual) {
                ...
                html += `<div class="req-categoria-titulo">${catAtual}</div>`;
            }
            ...
            <div class="req-card">
                <div>
                    <h4 style="margin:0; color:#333; display:flex; align-items:center;">
                        ${d.titulo} <span class="req-badge-pts">${d.pontos || 0} pts</span>
                    </h4>
                    <p style="color: ${corStatus}; font-size: 0.85rem; margin-top:5px; max-width: 90%;">
                        ${d.descricao || ''}
                    </p>
                </div>
                <button ${btnDisabled} onclick="abrirModalEnvio('${d.titulo}', ${d.pontos || 0})" class="btn-enviar-req">${txtBotao}</button>
            </div>`;
        });

        divLista.innerHTML = html;

    } catch (e) { divLista.innerHTML = "Erro ao carregar."; console.error(e); }
}

window.abrirModalEnvio = (titulo, pontos) => {
    document.getElementById('modal-req-titulo').innerText = titulo;
    document.getElementById('modal-req-nome').value = JSON.stringify({ titulo, pontos });
    document.getElementById('modal-req-relato').value = "";
    document.getElementById('modal-req-foto').value = "";
    document.getElementById('modal-envio-req').classList.remove('hidden');
}
...
window.expandirFoto = (fotoBase64) => {
    Swal.fire({
        imageUrl: fotoBase64,
        imageAlt: 'Comprovação do Requisito',
...
// --- ANIMAÇÃO DE CONFETES 5 ESTRELAS ---
window.dispararConfetes = () => {
